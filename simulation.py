...
now_price = json.loads(read_influx.read_with_range_pivot(
    ...
    (simulation_now_time + timedelta(minutes=1)).isoformat()+"Z")) \
    [0]["close"]
...
hour_list = [(simulation_now_time - timedelta(hours=i))
             ...
             for i in range(19,0,-1)]
start_time = simulation_now_time - timedelta(minutes=19)
...
for date in dates_list:
    ...
    df = pd.DataFrame(temp_data, columns=["_time", "close", "da20"])
...
for hour in hour_list:
    ...
    temp_data = json.loads(temp)
    df = pd.DataFrame(temp_data, columns=["_time", "close"]) #20시간 전부터 시간 별 종가 정보
    dataframes.append(df)
last_19hour = pd.concat(dataframes, ignore_index=True)
last_19min = pd.read_json(read_influx.read_with_range_pivot(
    measure_name, start_time.isoformat()+"Z", simulation_now_time.isoformat()+"Z"))[['_time', 'close']]

bollinger = bollinger()
bollinger.calc_bolinger_day(last_19days, now_price, simulation_now_time.isoformat())
bollinger.calc_bolinger_hour(
    last_19hour, now_price, simulation_now_time.isoformat())
bollinger.calc_bolinger_min(
    last_19min, now_price, simulation_now_time.isoformat())
